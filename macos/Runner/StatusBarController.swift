...
class StatusBarController {
    ...
    init(_ popover: NSPopover) {
        ...
        if let statusBarButton = statusItem.button {
            ...
            if let icon = NSImage(systemSymbolName: "xmark.rectangle", accessibilityDescription: "FxClashX") {
    ...
    func updateIcon(isVpnConnected: Bool) {
        if let button = statusItem.button {
            ...
            if let icon = NSImage(systemSymbolName: imageName, accessibilityDescription: "FxClashX") {
                let config = NSImage.SymbolConfiguration(scale: .large)
                let configuredIcon = icon.withSymbolConfiguration(config)
